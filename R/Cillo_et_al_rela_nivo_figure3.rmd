...
## Modules across pseudotime bins 
...
conf_ints <- fig3_data[[1]] %>%
  ...
  split(.$gene_sets) %>%
  lapply(.,function(x) {
    mod_res <- lmer(values~time_tx+(1|patient_id)-1,data=x)
    mod_conf <- confint(lmer(values~time_tx+(1|patient_id)-1,data=x))
...
# Plots 
p5 <- conf_ints[[1]] %>%
  mutate(tx=as.factor(recode(group,
                             time_txBaseline="Baseline",
                             time_txcombo="Rela+Nivo",
                             time_txnivo="Nivo",
                             time_txrela="Rela"))) %>%
  mutate(tx=factor(tx,levels=c("Baseline",
                               ...
                               "Nivo",
                               "Rela+Nivo"))) %>%
  ggplot(.,aes(x=tx,y=fixed_eff,ymin=`2.5 %`,ymax=`97.5 %`,
               fill=tx)) +
  geom_crossbar() +
  theme_bw() +
  theme(legend.position = "none") +
  xlab("Treatment Group") +
  ylab("Terminal exhaustion module") +
